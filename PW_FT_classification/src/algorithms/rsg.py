...
class RSG(pl.LightningModule):
    ...
    def __init__(self, conf, train_class_counts, id_to_labels, head_lists, epoch_thresh, **kwargs):
        """
        Initializes the RSG model.
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Forward pass
        feats, cesc_loss, total_mv_loss, combine_target = self.net.feature(
            data, epoch=self.current_epoch, batch_target=label_ids, phase_train=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Forward pass
        feats = self.net.feature(data, phase_train=False)
        logits = self.net.classifier(feats)
