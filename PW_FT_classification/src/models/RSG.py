# RSG functions adapted from https://github.com/Jianf-Wang/RSG.git
...
class RSG(nn.Module):
    def __init__(self, n_center = 3, feature_maps_shape = [32, 16, 16], num_classes=10, 
                 contrastive_module_dim = 128, head_class_lists = [], transfer_strength = 1.0, 
                 epoch_thresh = 100):
    ...
    def compute_cesc_loss(self, feature_maps, c, gamma, target, epoch):
        ...
        if epoch <= self.epoch_thresh:
         ...
         if num % 2: #batch size is odd and last feature map needs to be removed for feature1 and feature 2 to be same shape
           num_even = num - 1
         else: 
           num_even = num

         feature1 = feature_maps[0:num_even//2, :, :, :]
         feature2 = feature_maps[num_even//2:num_even, :, :, :]

         target1 = target[0 : num_even//2]
         target2 = target[num_even//2 : num_even]
    ...
    def compute_mv_loss(self, origin_feature, origin_center, target_center, target_features, gamma_head, target, gamma_tail):
        ...
        first_term = torch.sum(torch.abs(torch.norm(var_map_t.view(-1,self.C), dim=1) - torch.norm(var_map.view(-1, self.C), dim=1)))
        sec_term = torch.sum(torch.abs(target_features_norm * var_map_norm - torch.ones(target_features_norm.size(), device='cuda')))
        ave_loss = (first_term + sec_term)/(num)
        loss = F.cross_entropy(pair_pred, torch.zeros(num, device='cuda').long()) + ave_loss
    ...
    def forward(self, feature_maps, head_class_lists, target, epoch):
        ...
        head_class_lists_tensor = torch.tensor(head_class_lists, device='cuda')
        ...
        loss_mv_total = torch.zeros(loss_cesc.size(), device='cuda')
